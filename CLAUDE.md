...
Xwom 是一个集成通用 AI Agent 的下一代人工智能软件，采用 Electron 桌面应用架构。
...
**包命名规范**：`@xwom/*` 作用域（`@xwom/core`、`@xwom/shared`、`@xwom/ui`、`@xwom/electron`）
...
1. **类型 & 常量**：`@xwom/shared` 定义 IPC 通道名称常量和请求/响应类型
...
- **事件转换**：`convertSDKMessage()`（`@xwom/shared`）将 SDK 原始消息转为统一的 `AgentEvent` 类型
